...
@MapperScan(basePackages="com.zhangtao.dao")
...
//    private String readDataSourceSize;
    ...
    @Autowired
    @Qualifier("userDBWriteDataSource")
    private DataSource userDBWriteDataSource;
    @Autowired
    ...
    @Autowired
    @Qualifier("goodsDBReadDataSource")
    private DataSource goodsDBReadDataSource;
    @Autowired
    @Qualifier("quotesDBWriteDataSource")
    private DataSource quotesDBWriteDataSource;
    @Autowired
    @Qualifier("quotesDBReadDataSource")
    private DataSource quotesDBReadDataSource;
    @Autowired
    @Qualifier("financeDBWriteDataSource")
    private DataSource financeDBWriteDataSource;
    @Autowired
    @Qualifier("financeDBReadDataSource")
    private DataSource financeDBReadDataSource;
    @Autowired
    @Qualifier("queueDBWriteDataSource")
    private DataSource queueDBWriteDataSource;
    @Autowired
    @Qualifier("queueDBReadDataSource")
    private DataSource queueDBReadDataSource;
...
//	private DataSource readDataSource02;
    ...
    public SqlSessionFactory sqlSessionFactorys() throws Exception {
        ...
        try {
            ...
            // 读取配置
            sessionFactoryBean.setTypeAliasesPackage("com.zhangtao.domain");
    ...
    public AbstractRoutingDataSource roundRobinDataSouceProxy() {
        ...
        //否则切换数据源时找不到正确的数据源
        targetDataSources.put(DataSourceType.userDBwrite.getType(), userDBWriteDataSource);
...
//        targetDataSources.put(DataSourceType.userDBread.getType()+"2", readDataSource02);
        ...
        targetDataSources.put(DataSourceType.quotesDBwrite.getType(), quotesDBWriteDataSource);
        targetDataSources.put(DataSourceType.quotesDBread.getType(), quotesDBReadDataSource);
        targetDataSources.put(DataSourceType.financeDBwrite.getType(), financeDBWriteDataSource);
        targetDataSources.put(DataSourceType.financeDBread.getType(), financeDBReadDataSource);
        targetDataSources.put(DataSourceType.queueDBwrite.getType(), queueDBWriteDataSource);
        targetDataSources.put(DataSourceType.queueDBread.getType(), queueDBReadDataSource);
...
//                    return DataSourceType.userDBwrite.getType();
                    throw new NullPointerException("数据库路由时，决定使用哪个数据库源类型不能为空...");
                }

                System.out.println("使用数据库"+DataSourceType.valueOf(typeKey).getName());
                return DataSourceType.valueOf(typeKey).getType();


//                else if (typeKey.equals(DataSourceType.userDBread.getType())){
//                    System.err.println("使用数据库"+DataSourceType.userDBread.getName());
//                    return DataSourceType.userDBread.getType();
//                }
...
//                return DataSourceType.userDBread.getType()+(lookupKey+1);
//                System.out.println("使用数据库"+DataSourceType.userDBwrite.getName());
//                return DataSourceType.userDBwrite.getType();
            }
        };

        proxy.setDefaultTargetDataSource(userDBWriteDataSource);//默认库
